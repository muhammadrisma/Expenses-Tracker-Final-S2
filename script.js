...
function render() {
    balanceEl.innerHTML = `Rp. ${state.balance.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
    incomeEl.innerHTML = `Rp. ${state.income.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
    expenseEl.innerHTML = `Rp. ${state.expense.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
    ...
    for (var i = 0; i < state.transactions.length; i++) {
        ...
        amountEl.innerHTML = `Rp ${item.amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
